...
<body class="bg-[#F8FAFC] font-sans min-h-screen flex flex-col">
    ...
    <main class="max-w-3xl mx-auto mt-8 px-4 w-full flex-grow mb-12">
        ...
        <!-- FORM UTAMA -->
        <form id="formPraSurvei" action="{{ route('storeAlur8') }}" method="POST" class="space-y-6" novalidate>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <!-- Analisis Aset yang dimiliki calon debitur -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Analisis Aset yang dimiliki calon debitur <span class="text-red-500">*</span>
                    </label>
                    <textarea name="analisis_aset" rows="6" required placeholder=""
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0082CB]">{{ old('analisis_aset', $capital->analisis_aset ?? '') }}</textarea></div>
            </div>
    ...
    <script>
    // Validasi form khusus halaman Capital (analisis_aset)
    const formPraSurvei = document.getElementById('formPraSurvei');
    formPraSurvei.addEventListener('submit', function(event) {
        const analisisAset = formPraSurvei.querySelector('textarea[name="analisis_aset"]');
        let isValid = true;
        let errorMessage = 'Mohon lengkapi semua pertanyaan yang bertanda (*)';

        // Validasi textarea analisis_aset
        if (!analisisAset || analisisAset.value.trim() === '') {
            isValid = false;
        }
