...
<body class="bg-[#F8FAFC] font-sans min-h-screen flex flex-col">
        ...
        <form id="formPraSurvei" action="{{ route('storeStep10') }}" method="POST" class="space-y-6">
            ...
            <div class="flex justify-between items-center pt-2">
                ...
                <div class="flex items-center gap-3">  
                    ...
                    <button type="submit" class="bg-[#0082CB] text-[#FFFFFF] border-2 border-[#0082CB] px-8 py-2 rounded-lg text-sm font-semibold hover:bg-[#006FB0] transition shadow-md">
                        Kirim
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        document.getElementById('formPraSurvei').addEventListener('submit', function(e) {
            ...
            // Tampilkan Pop-up Modal dengan Efek Blur di Background
            Swal.fire({
                title: 'Berhasil!',
                text: 'Data Anda berhasil tersimpan!',
                icon: 'success',
                showConfirmButton: false, // Hilangkan tombol OK agar otomatis tertutup sendiri
                timer: 1800,              // Durasi pop-up muncul (1.8 detik)
                width: '320px',           // Ukuran kotak pop-up kecil/compact
                padding: '1.25rem',       // Jarak dalam agar rapi
                backdrop: `
                    rgba(0, 0, 0, 0.4)
                ...
                didOpen: () => {
                    // Redupkan form utama di belakang agar fokus ke pop-up
                    document.querySelector('main').style.opacity = '0.3';
                }
            });

            // Begitu timer pop-up selesai, langsung sembunyikan seluruh body 
            // agar tidak ada jeda form terlihat, lalu kirim form ke server
            setTimeout(function() {
