    ...
    <main class="max-w-3xl mx-auto mt-8 px-4 w-full flex-grow mb-12">
        ...
        <form id="formPraSurvei" action="{{ route('storeAlur11') }}" method="POST" class="space-y-6" novalidate>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
                <h3 class="text-md font-bold text-[#0A3370] border-b pb-2 mb-2">Kelengkapan Berkas</h3>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Analisis Kelengkapan Berkas <span class="text-red-500">*</span>
                    </label>
                    <textarea name="analisis_kelengkapan_berkas" rows="6" placeholder="" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0082CB]">{{ old('analisis_kelengkapan_berkas', $kelengkapan->analisis_kelengkapan_berkas ?? '') }}</textarea>
                </div>
                    ...
                    <div class="space-y-3 text-sm text-gray-700">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="apakah_badan_usaha" value="YA" 
                                {{ (old('apakah_badan_usaha', $kelengkapan->apakah_badan_usaha ?? '') == 'YA') ? 'checked' : '' }} 
                                class="accent-[#0082CB]" required>
                            <span>YA</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="apakah_badan_usaha" value="TIDAK" 
                                {{ (old('apakah_badan_usaha', $kelengkapan->apakah_badan_usaha ?? '') == 'TIDAK') ? 'checked' : '' }} 
                                class="accent-[#0082CB]" required>
                            <span>TIDAK</span>
                        </label>
                    </div>
...
<script>
    // Validasi form khusus halaman Kelengkapan Berkas & Apakah Badan Usaha
    const formPraSurvei = document.getElementById('formPraSurvei');
    formPraSurvei.addEventListener('submit', function(event) {
        const analisisBerkas = formPraSurvei.querySelector('textarea[name="analisis_kelengkapan_berkas"]');
        ...
        // Validasi textarea analisis_kelengkapan_berkas
        if (!analisisBerkas || analisisBerkas.value.trim() === '') {
            isValid = false;
        }
        // Validasi radio button apakah_badan_usaha
        else if (!selectedBadanUsaha) {
            isValid = false;
        }
