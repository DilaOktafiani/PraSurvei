...
<body class="bg-[#F8FAFC] font-sans min-h-screen flex flex-col">
    ...
    <main class="max-w-3xl mx-auto mt-8 px-4 w-full flex-grow mb-12">
        ...
        <!-- FORM UTAMA -->
        <form id="formPraSurvei" action="{{ route('storeAlur10') }}" method="POST" class="space-y-6" novalidate>    
            @csrf
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <p class="text-xs text-gray-500 mt-0.5">Apabila take over isikan analisnya</p>
...
<script>
    // Validasi form khusus halaman Analisis Take Over
    const formPraSurvei = document.getElementById('formPraSurvei');
    formPraSurvei.addEventListener('submit', function(event) {
        const analisisTakeOver = formPraSurvei.querySelector('textarea[name="analisis_take_over"]');
        let isValid = true;
        let errorMessage = 'Mohon lengkapi semua pertanyaan yang bertanda (*)';

        // Validasi textarea analisis_take_over
        if (!analisisTakeOver || analisisTakeOver.value.trim() === '') {
            isValid = false;
        }
