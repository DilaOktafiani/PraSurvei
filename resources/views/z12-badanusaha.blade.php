...
<body class="bg-[#F8FAFC] font-sans min-h-screen flex flex-col">
    ...
    <main class="max-w-3xl mx-auto mt-8 px-4 w-full flex-grow mb-12">
        ...
        <!-- FORM UTAMA -->
        <form id="formPraSurvei" action="{{ route('storeAlur12') }}" method="POST" class="space-y-6" novalidate>
            @csrf
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="analisa_badan_usaha" rows="6" placeholder="" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0082CB]">{{ old('analisa_badan_usaha', $kelengkapanBadanUsaha->analisa_badan_usaha ?? '') }}</textarea>
                </div>
            </div>
...
<script>
    // Validasi form khusus halaman Analisa Berkas Badan Usaha
    const formPraSurvei = document.getElementById('formPraSurvei');
    formPraSurvei.addEventListener('submit', function(event) {
        const analisaBadanUsaha = formPraSurvei.querySelector('textarea[name="analisa_badan_usaha"]');
        let isValid = true;
        let errorMessage = 'Mohon lengkapi semua pertanyaan yang bertanda (*)';

        // Validasi textarea analisa_badan_usaha
        if (!analisaBadanUsaha || analisaBadanUsaha.value.trim() === '') {
            isValid = false;
        }
